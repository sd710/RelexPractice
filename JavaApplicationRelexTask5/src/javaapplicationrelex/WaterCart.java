...
*/
//--5 задача--
// добавлено вычисление премещение от одной клумбы к другой.
...
public class WaterCart {
    ...
    // --5 задача--
    // время перемещения теперь зависит от местоположения машины.
    //Если индекс машины не первый в списке (не 0) значит,
    //время перемещение = сколько ехать до клумбы B - сколько ехать до клумбы A
    private int DifferenceMoveTo(Sensor flowers){
        int index = listBedsSensors.indexOf(flowers);
        if (index != 0){
           return listBedsSensors.get(index).getTimeMoveToFlowerbed() - listBedsSensors.get(index - 1).getTimeMoveToFlowerbed();
        }
        return flowers.getTimeMoveToFlowerbed();
    }
    
    /* перемещение к заданной клумбе */ //--3 задача-- сколько займет дорога к конкретной клумбе
    public void goToFlowerbed(Sensor flowers){   
        System.out.printf("Я еду к клумбе %s... Дорога займёт %d мин...\n", flowers.getNameFlowerbed(), DifferenceMoveTo(flowers));
